...
Approximately 44\% of the images originate from our own team \textit{HULKs}; the remainder was collected from \textit{B-Human}, \textit{SPQR}, \textit{Nao Devils}, \textit{BerlinUnited}, and \textit{Dutch Nao Team}.
...
\begin{figure}
	...
	\caption{Distribution over the light source (red shades), reflection/shadow (purple shades), line (green shades), and field (orange shades) conditions. The top plot corresponds to the full dataset, while the bottom plot describes the labeled subset.}\label{fig:dataDistribution}
\end{figure}
...
Because pixel-wise annotation is costly, the labeled subset was sampled to reflect the broader condition distribution, making it suitable for representative supervised experiments and condition-specific evaluations.
...
The annotation process was aided with \gls{cvat} and \gls{sam}.
...
The annotated subset can train and evaluate segmentation models, while the large pool of unlabeled images enables custom annotation strategies and semi- or self-supervised learning~\cite{goettsch2026lightweight}.
